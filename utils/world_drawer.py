...
class Drawer:
    ...
    def __init__(self, screen_width=1920, screen_height=1080,
                 field_line_size=4, scale_rate=1, font_name='fonts/font.otf',
                 font_size=30, world_width=9, world_height=9):
        self.args = dict()
        ...
        self.field_line_len = int(screen_height * 0.95 * scale_rate)
        self.cell_size = self.field_line_len // world_height
        self.entity_size = int(self.cell_size * 0.95)
    ...
    def _draw_field(self, screen):
        for i in range(self._world_width + 1):
            x = self.offset_left + self.cell_size * i - 1
            pygame.draw.line(
                screen, (0, 0, 0),
                (x, self.offset_top), (x, self.offset_bottom),
                self.field_line_size
            )

        for i in range(self._world_height + 1):
            y = self.offset_top + self.cell_size * i - 1
            pygame.draw.line(
                screen, (0, 0, 0),
                (self.offset_left, y), (self.offset_right, y),
                self.field_line_size
            )
        ...
        for row in field:
            for entity in row:
                if entity.get_state() != ENTITIES_STATES['empty']:
                    x = self.offset_left + self.cell_size * entity.get_x()
                    y = self.offset_top + self.cell_size * entity.get_y()
                    x += (self.cell_size - self.entity_size) / 2
                    y += (self.cell_size - self.entity_size) / 2
    ...
    def draw_result_table(self, screen, game_results: list,
                          font_name='fonts/font.otf', font_color=(0, 0, 0)):
        screen.fill((255, 255, 255))

...
def update_entity(entity: DynamicEntity, action, field_width=9, field_height=9):
    new_y = entity.get_y() + action[0]
    new_x = entity.get_x() + action[1]

    if 0 <= new_y < field_height and 0 <= new_x < field_width:
        entity.set_x(new_x)
        entity.set_y(new_y)
...
def update_field(field, entity: DynamicEntity, action: tuple, world_generator,
                 holes, amount_of_cheese_to_win=5,
                 amount_of_do_nothing_actions_to_loose=3,
                 field_width=9, field_height=9) -> tuple:
